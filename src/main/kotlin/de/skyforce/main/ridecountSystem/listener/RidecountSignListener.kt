...
class RidecountSignListener(
    ...
    companion object {
        ...
        private const val PREFIX = "[Ridecount] "
    }
    ...
    fun onSignChange(event: SignChangeEvent) {
        ...
        if (!player.hasPermission(PERMISSION_CREATE)) {
            ...
            player.sendMessage("${PREFIX}Du hast keine Berechtigung, Ridecount-Schilder zu erstellen.")
        ...
        if (line2.isBlank()) {
            event.isCancelled = true
            player.sendMessage("${PREFIX}Bitte gib in Zeile 3 den Namen der Attraktion an.")
            val block = event.block
            plugin.server.scheduler.runTask(plugin) { _ -> block.type = Material.AIR }
            return
        }

        player.sendMessage("${PREFIX}Ridecount-Schild fuer $line2 erfolgreich erstellt.")
    }
